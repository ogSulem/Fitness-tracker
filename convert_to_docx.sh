...
if [[ -f "$OUTPUT" ]]; then
    ...
    echo "   1. Обновить оглавление (ПКМ на содержании → «Обновить поле» → «Обновить всё»)"
    echo "   2. Проверить нумерацию страниц (должна начинаться с 'Содержание', не с титула)"
    echo "   3. Диаграммы Mermaid заменить скриншотами — они отображаются как код"
    echo "   4. Проверить переносы строк в таблицах"
    echo "   5. Проверить формулы (3.1)–(3.3) — при необходимости вставить через Word Equation"
else
