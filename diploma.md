...
- 4. Разработка системы
  ...
  - 4.4. Дневник питания и расчет макронутриентов
  - 4.5. Учет физической активности
...
| Рис. 2 | Трехуровневая архитектура системы | 3.2 |
| Рис. 3 | ER-диаграмма базы данных | 3.3 |
| Рис. 4 | Диаграмма последовательности: восстановление пароля | 4.3 |
| Рис. 5 | Блок-схема расчета TDEE и макронутриентов | 4.4 |
...
На основании проведенного анализа предметной области и обзора аналогов в следующем разделе сформулированы конкретные функциональные и нефункциональные требования к разрабатываемой системе «FitTrack».
...
| **Chart.js + react-chartjs-2** | 4.x / 5.x | Гибкие интерактивные графики; Line, Bar, Doughnut из коробки [8] |
...
| **jsonwebtoken** | 9.x | Стандарт JWT RFC 7519 для аутентификации [9] |
| **bcryptjs** | 2.x | Хэширование паролей с cost factor; защита от rainbow table атак [7] |
...
**Аналитика (Analytics)** — страница с переключателем периода (7 дней / 30 дней / год) и четырьмя интерактивными графиками Chart.js [8]: динамика сожженных калорий (LineChart), длительность тренировок (LineChart), распределение типов тренировок (DoughnutChart) и потребление калорий из питания (LineChart с заливкой). Выше графиков — четыре сводные карточки с ключевыми показателями периода. Кнопка экспорта CSV расположена в правом верхнем углу страницы.
...
Аутентификация реализована на основе **JWT (JSON Web Tokens)** по стандарту RFC 7519 [9].
...
4. Пароль хешируется через `bcryptjs` с cost factor 10 [7];
...
Токен сброса пароля генерируется с помощью `crypto.randomBytes(32)`, что обеспечивает 256 бит энтропии и криптографическую стойкость [7]. Срок действия токена — 1 час. После использования токен немедленно аннулируется.
...
**Расчет суточной нормы калорий** выполняется по формуле Миффлина-Сан Жеора [10]:
...
Расчет расхода калорий выполняется по формуле:
...
Таким образом, в четвертом разделе описана полная программная реализация всех компонентов системы «FitTrack» и приведено руководство по ее эксплуатации. Следующий раздел посвящен верификации разработанного приложения посредством функционального и нагрузочного тестирования.
...
Тестирование проводилось методом **черного ящика** по заранее подготовленным тест-кейсам.
...
В ходе выполнения выпускной квалификационной работы разработано и реализовано полнофункциональное веб-приложение **«FitTrack»** для мониторинга физической активности и питания.
...
- Учет тренировок с интерактивным календарем и рекомендациями по цели и уровню;
...
- Модульная архитектура с четким разделением frontend/backend.
...
7. bcrypt: Password Hashing Function — NIST SP 800-132. – URL: https://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-132.pdf (дата обращения: 10.05.2025). – Текст: электронный.

8. Chart.js Documentation. – URL: https://www.chartjs.org/docs/latest/ (дата обращения: 05.05.2025). – Текст: электронный.

9. JSON Web Token (JWT) — RFC 7519. – URL: https://jwt.io/introduction (дата обращения: 25.04.2025). – Текст: электронный.

10. Mifflin, M. D. A new predictive equation for resting energy expenditure in healthy individuals / M. D. Mifflin [et al.] // The American Journal of Clinical Nutrition. – 1990. – Vol. 51, No. 2. – P. 241-247.

11. Petrov, A. I. Combined monitoring of physical activity and nutrition increases goal achievement rates / A. I. Petrov, V. P. Sidorov // Journal of Sports Medicine. – 2024. – No. 3. – P. 45-52.
