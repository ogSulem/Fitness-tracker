...
6. **Отсутствие аутентификации на маршруте POST `/api/recommendations`** — эндпоинт для создания рекомендаций тренировок не был защищен JWT-middleware, что позволяло любому желающему без авторизации добавлять или перезаписывать данные. Исправлено: `auth` middleware добавлен ко всем трем маршрутам роутера (`GET /`, `GET /:goal/:level`, `POST /`).

7. **Ошибка CORS для origin `http://localhost:5001/` (trailing slash)** — браузер добавляет завершающий слеш к origin (`http://localhost:5001/`), тогда как в белом списке CORS хранится строка без слеша (`http://localhost:5001`). Строгое сравнение `===` давало `false`, и сервер возвращал ошибку `403 CORS: origin not allowed`. Одновременно origin сервера (`localhost:5001`) вообще не был в whitelist, что создавало проблемы при production-режиме, когда сервер отдает собранный React. Исправлено: добавлена функция `stripSlash()`, нормализующая оба значения перед сравнением; функция `buildAllowedOrigins()` автоматически включает в whitelist `http://localhost:{PORT}`, `http://127.0.0.1:{PORT}` и значение `CLIENT_URL` из переменных окружения.

8. **Неправильная формула BMR в `CalorieCalculator.js` и `Home.js`** — в обоих компонентах применялась формула Харриса–Бенедикта (1984, пересмотренная версия Роза–Шизгала), тогда как в дипломной работе задекларировано использование более точной формулы Миффлина–Сан Жеора (1990). Коэффициенты существенно отличаются: у Харриса–Бенедикта для мужчин — `88.362 + 13.397×m + 4.799×h − 5.677×a`, тогда как у Миффлина–Сан Жеора — `10×m + 6.25×h − 5×a + 5`. Помимо несоответствия диплому, алгоритм корректировки нормы калорий по цели использовал мультипликаторы (×0,85 и ×1,15 от TDEE) вместо абсолютных значений (TDEE − 500 и TDEE + 300 ккал), что также расходилось с описанием в разделе 3.4. Исправлено: оба компонента переведены на формулу Миффлина–Сан Жеора; корректировка по цели заменена на стандартные фиксированные значения.

9. **CSV-экспорт в `Analytics.js`: пустая колонка «Примечание»** — поле комментария к тренировке в модели MongoDB называется `comment`, однако в функции `exportCSV()` обращение шло к `w.notes`, которого не существует. Это приводило к тому, что столбец «Примечание» в экспортируемом CSV всегда оставался пустым. Исправлено: `w.notes` заменено на `w.comment`.

10. **Отсутствие rate limiting на legacy-эндпоинтах в `users.js`** — маршруты `/api/users/weight-history`, `/api/users/goals` (GET/POST) и `/api/users/workouts` (GET/POST) не имели rate limiting, что создавало возможность для DoS-атаки через перегрузку базы данных. Исправлено: ко всем перечисленным эндпоинтам добавлен `profileLimiter` (30 запросов / 15 мин / IP).
