...
В современном мире растет интерес к здоровому образу жизни и систематическим физическим нагрузкам. Согласно данным Всемирной организации здравоохранения, недостаточная физическая активность является четвертым по значимости фактором риска глобальной смертности [6]. При этом 23% взрослого населения и более 81% подростков не выполняют минимальные рекомендации ВОЗ по двигательной активности.

Рынок цифровых инструментов для отслеживания здоровья демонстрирует устойчивый рост на 17,6% в год [5]. Особую востребованность приобретают персонализированные системы, учитывающие не только физическую активность, но и рацион питания пользователя — поскольку именно совокупность тренировок и питания определяет достижение фитнес-целей.
...
Задачи:
1. Провести анализ предметной области, выявить недостатки существующих аналогов, определить функциональные и нефункциональные требования к системе и обосновать выбор технологического стека на основе сравнительного анализа альтернатив;
2. Спроектировать трехуровневую клиент-серверную архитектуру с разделением слоев представления, бизнес-логики и хранения данных; разработать схему базы данных, REST API и прототип пользовательского интерфейса;
3. Реализовать полнофункциональное веб-приложение, включая персонализированный алгоритм расчета суточной нормы калорий по формуле Миффлина–Сан Жеора, безопасную систему аутентификации на JWT, двуязычный интерфейс (RU/EN) с переключением без перезагрузки и модуль визуализации и аналитики с экспортом данных;
4. Провести комплексное тестирование: функциональное (метод черного ящика), нагрузочное (Apache JMeter) и тестирование безопасности по методологии OWASP Top 10; подтвердить соответствие системы заявленным требованиям.
...
Современные исследования убедительно подтверждают ключевую роль регулярной физической активности и сбалансированного питания в сохранении здоровья. По данным ВОЗ, физически активные люди имеют на 35% более низкий риск сердечно-сосудистых заболеваний, на 30% — риск сахарного диабета 2-го типа [6].

Российский рынок фитнес-приложений в 2024 году оценивается более чем в 12 млрд рублей и продолжает расти [5]. Пандемия COVID-19 дала дополнительный импульс цифровизации фитнеса: число пользователей, тренирующихся дома с использованием приложений, выросло на 70% в 2020–2022 годах и сохраняет высокие показатели.
...
| | 7.4 | Двуязычный интерфейс (русский / английский) с переключением без перезагрузки | ✅ Реализовано |
...
В качестве СУБД выбрана MongoDB — документоориентированная база данных с гибкой схемой JSON-документов и нативным горизонтальным масштабированием (Sharding). В отличие от реляционных СУБД (PostgreSQL, MySQL), MongoDB оптимально подходит для хранения разнородных фитнес-данных с переменным набором полей.
...
|---|---|---|
| React | 18.2 | Компонентная архитектура, виртуальный DOM, Context API для управления состоянием [9] |
| Tailwind CSS | 3.x | Utility-first подход; встроенная поддержка `darkMode: 'class'`; производительнее CSS-in-JS [18] |
| Chart.js + react-chartjs-2 | 4.x / 5.x | Гибкие интерактивные графики; Line, Bar, Doughnut из коробки [12] |
| React Router | v6 | Декларативная маршрутизация; защищенные маршруты [17] |
| Axios | 1.x | Удобный HTTP-клиент с перехватчиками (interceptors) для обновления токенов [10] |
| Day.js | 1.x | Легкая альтернатива Moment.js для работы с датами [13] |
...
| Express | 4.x | Минималистичный HTTP-фреймворк; богатая экосистема middleware [7] |
| MongoDB | 7.x | Документоориентированная СУБД; гибкая схема для разнородных фитнес-данных [8] |
| Mongoose | 7.x | ODM для MongoDB; валидация схемы на уровне модели [15] |
| jsonwebtoken | 9.x | Стандарт JWT RFC 7519 для аутентификации [14] |
| bcryptjs | 2.x | Хэширование паролей с cost factor; защита от rainbow table атак [11] |
...
| Docker + Docker Compose | 24+ / 2.x | Контейнеризация приложения и MongoDB; воспроизводимое окружение на VPS |
| Nginx | 1.26+ | Reverse proxy; терминирование TLS/HTTPS; проксирование на порт 5001 |

Обоснование выбора MongoDB в сравнении с реляционными аналогами: в отличие от PostgreSQL и MySQL, MongoDB не требует фиксированной схемы таблиц — каждый документ в коллекции может иметь разный набор полей. Это критически важно для хранения фитнес-данных: пользователи добавляют разные типы тренировок (бег, силовая, плавание), каждый из которых может иметь специфические атрибуты. Горизонтальное масштабирование через Sharding реализовано нативно. В рамках проекта MongoDB запускается в Docker-контейнере (образ mongo:7) с именованным томом для сохранения данных; при необходимости облачного развертывания возможно подключение к MongoDB Atlas.
...
| Развертывание на VPS | ✅ Docker + Nginx | ⚠️ Требует настройки | ⚠️ Требует настройки | ❌ Сложно |
...
│       │   └── NotificationContext.js, LanguageContext.js
│       ├── hooks/
...
Схема развертывания системы на VPS-сервере с использованием Docker представлена на рисунке 3.
...
flowchart LR
    ...
    subgraph VPS["🖥️ VPS-сервер (Linux, Ubuntu 22.04)"]
        Nginx["Nginx\nreverse proxy\nHTTPS / порт 443"]
        subgraph Docker["🐳 Docker Compose"]
            APP["fittrack-app\nNode.js + Express\n+ React static\nпорт 5001"]
            MONGO["fittrack-mongo\nMongoDB 7\nтом mongo_data"]
        end
    end

    Browser -->|"HTTPS"| Nginx
    Nginx -->|"HTTP proxy → :5001"| APP
    APP -->|"MongoDB Wire Protocol"| MONGO
```

Рисунок 3. Диаграмма развертывания системы

Для развертывания в production используется VPS-сервер под управлением Linux (Ubuntu 22.04) с установленным Docker и Docker Compose. Multi-stage Dockerfile собирает React SPA на этапе client-build и копирует готовые статические файлы в production-образ сервера; единый контейнер fittrack-app обслуживает как REST API, так и статику React. Контейнер fittrack-mongo запускает MongoDB 7 с именованным томом mongo_data для сохранения данных между перезапусками. Перед контейнерами развертывается Nginx в качестве reverse proxy: он принимает входящие HTTPS-соединения на порту 443 (с TLS-сертификатом Let's Encrypt) и перенаправляет трафик на порт 5001 приложения. Такая схема обеспечивает изоляцию сервисов, воспроизводимость окружения и простоту масштабирования.
...
Каждая запись `FoodEntry` соответствует одному приему пищи в рамках одной даты и содержит вложенный массив продуктов.
...
Аналитика (Analytics) — страница с переключателем периода (7 дней / 30 дней / год) и четырьмя интерактивными графиками Chart.js [12]: динамика сожженных калорий (LineChart), длительность тренировок (LineChart), распределение типов тренировок (DoughnutChart) и потребление калорий из питания (LineChart с заливкой). Выше графиков — четыре сводные карточки с ключевыми показателями периода. Кнопка экспорта CSV расположена в правом верхнем углу страницы.
...
// ... еще 5 маршрутов (users, workouts, nutrition, goals, recommendations) ...
...
Контекст аутентификации (`AuthContext.js`) использует хуки React (`useState`, `useEffect`) для управления состоянием, что позволяет реализовать реактивное обновление UI без перезагрузки страницы [2].
...
4. Пароль хешируется через `bcryptjs` с cost factor 10 [11];
...
Токен сброса пароля генерируется с помощью `crypto.randomBytes(32)`, что обеспечивает 256 бит энтропии и криптографическую стойкость [11]. Срок действия токена — 1 час. После использования токен немедленно аннулируется.
...
Расчет суточной нормы калорий выполняется по формуле Миффлина–Сан Жеора [4], являющейся наиболее точной для вычисления основного обмена веществ (BMR):

для мужчин:

BMR = 10 × m + 6,25 × h − 5 × a + 5,    (3.1)

для женщин:

BMR = 10 × m + 6,25 × h − 5 × a − 161,    (3.2)
...
flowchart TD
    ...
    GenderCheck -->|Мужчина| BMR_M["BMR = 10 × вес\n+ 6.25 × рост\n− 5 × возраст + 5"]
    GenderCheck -->|Женщина| BMR_F["BMR = 10 × вес\n+ 6.25 × рост\n− 5 × возраст − 161"]
...
Тема управляется через `ThemeContext.js` и переключается с анимацией View Transitions API [19] — новая тема «выплескивается» расширяющимся кругом из точки клика на кнопку:
...
Система локализации реализована через `LanguageContext.js`. Словарь переводов содержит ключи для всех пользовательских строк: заголовков, меток форм, кнопок, мотивационных цитат и уведомлений на двух языках. Хук `useLang()` предоставляет компонентам функцию `t(key)`, возвращающую строку для текущего языка. Кнопка переключения языка (EN / RU) расположена в шапке рядом с переключателем темы. Выбор языка сохраняется в `localStorage` и применяется без перезагрузки страницы. При смене языка на «English» также переключается локаль библиотеки `dayjs`, что обеспечивает корректное отображение дат (день недели, месяц) на выбранном языке.

Система уведомлений:

Реализована через Web Notifications API [20] (браузерные уведомления). Пользователь указывает время приема пищи и тренировки; уведомление планируется через `setTimeout` до следующего наступления указанного времени. In-app уведомления (toast) реализованы через `NotificationContext.js` и отображаются в правом верхнем углу с автоматическим скрытием.

### 3.8. Руководство пользователя

Для развертывания и запуска приложения «FitTrack» предусмотрено два способа.

Способ 1. Запуск через Docker Compose (рекомендуется для production)

Требования к окружению: Docker 24+ и Docker Compose 2.x.

Установка и запуск:
1. Клонировать репозиторий: `git clone https://github.com/ogSulem/Fitness-tracker.git`;
2. В корне проекта создать файл `.env` с переменными окружения `JWT_SECRET` (произвольная строка-секрет) и, при необходимости, `CLIENT_URL` (URL фронтенда, если он развернут отдельно);
3. Выполнить команду `docker compose up -d --build` — Docker автоматически соберет multi-stage образ (сборка React → копирование статики → Node.js production) и запустит два контейнера: fittrack-app и fittrack-mongo;
4. Приложение доступно по адресу `http://localhost:5001`.

Способ 2. Запуск в режиме разработки без Docker

Требования к окружению: Node.js 18+, npm 9+, запущенный экземпляр MongoDB (локальный или удаленный).
...
| 26 | Переключение языка интерфейса (RU → EN) | Все надписи переключаются на английский без перезагрузки; даты отображаются на английском | ✅ Пройден |
...
5. Неправильный порядок маршрутов в `server/routes/workouts.js` — маршрут `GET /:id` был объявлен раньше специфичных маршрутов `GET /date/:date` и `GET /range/:start/:end`. Вследствие этого Express сопоставлял строку `"date"` с параметром `:id`, возвращал ошибку MongoDB CastError и статус 404. Исправлено: специфичные маршруты перемещены выше параметрических.

6. Отсутствие аутентификации на маршруте POST `/api/recommendations` — эндпоинт для создания рекомендаций тренировок не был защищен JWT-middleware, что позволяло неавторизованным пользователям добавлять или перезаписывать данные. Исправлено: `auth` middleware добавлен ко всем маршрутам роутера.

7. Ошибка CORS для origin с завершающим слешем (`http://localhost:5001/`) — браузер добавляет завершающий слеш к origin, тогда как в белом списке хранилась строка без слеша. Строгое сравнение давало `false` и сервер отвечал ошибкой `403 CORS: origin not allowed`. Исправлено: добавлена функция `stripSlash()`, нормализующая оба значения перед сравнением.

8. Неправильная формула BMR в `CalorieCalculator.js` и `Home.js` — применялась формула Харриса–Бенедикта вместо задекларированной в работе формулы Миффлина–Сан Жеора. Алгоритм корректировки нормы калорий по цели использовал мультипликаторы вместо абсолютных значений (TDEE − 500 и TDEE + 300 ккал). Исправлено: оба компонента переведены на формулу Миффлина–Сан Жеора.

9. CSV-экспорт в `Analytics.js`: пустая колонка «Примечание» — поле комментария в модели называется `comment`, но в функции `exportCSV()` обращение шло к несуществующему `w.notes`. Исправлено: `w.notes` заменено на `w.comment`.

10. Отсутствие rate limiting на ряде маршрутов в `users.js` — маршруты weight-history, goals и workouts не имели ограничения запросов, что создавало возможность DoS-атаки. Исправлено: добавлен `profileLimiter` (30 запросов / 15 мин / IP).
...
В ходе выполнения выпускной квалификационной работы разработано и введено в эксплуатацию полнофункциональное веб-приложение «FitTrack» для комплексного мониторинга физической активности и питания пользователя на базе стека MERN (MongoDB, Express, React, Node.js).

Все четыре задачи, поставленные во введении, выполнены в полном объеме.

1. Проведен сравнительный анализ предметной области: выявлены недостатки аналогов (MyFitnessPal, Strava, FatSecret, Cronometer) и сформулированы конкурентные преимущества разрабатываемой системы (таблица 1). Определены 14 функциональных (таблица 2) и 7 нефункциональных (таблица 3) требований к системе; обоснован выбор технологического стека — React [9], Node.js [1], Express [7], MongoDB [8] — на основе сравнительного анализа альтернатив (таблицы 4–6).

2. Спроектирована трехуровневая клиент-серверная архитектура с четким разделением слоев представления, бизнес-логики и хранения данных (рисунок 2); разработаны схема базы данных (рисунок 4), REST API (таблица 7) и прототип пользовательского интерфейса. Архитектурные решения обеспечивают независимое масштабирование слоев и упрощают тестирование.

3. Реализовано полнофункциональное веб-приложение: персонализированный алгоритм расчета суточной нормы калорий по формуле Миффлина–Сан Жеора (формулы 3.1–3.3) с учетом уровня активности и цели пользователя [4]; безопасная система аутентификации на JWT [14] с bcrypt-хэшированием паролей [11], rate limiting и криптографически стойким механизмом сброса пароля (256 бит, TTL 1 час); двуязычный интерфейс RU/EN на основе `LanguageContext.js` с переключением без перезагрузки; интерактивный дневник питания, учет тренировок с расчетом расхода калорий по МЕТ, модуль аналитики с четырьмя типами диаграмм Chart.js [12] за периоды 7/30/365 дней и экспортом данных в CSV.

4. Проведено комплексное тестирование: 26 функциональных тест-кейсов методом черного ящика (таблица 9) — все пройдены успешно; нагрузочное тестирование (Apache JMeter) подтвердило время ответа 220 мс при 100 одновременных пользователях — существенно ниже допустимого порога в 2–3 секунды (таблица 10); тестирование безопасности по 10 векторам атак OWASP Top 10 [16] подтвердило надежность реализованных защитных механизмов (таблица 11).

Результаты работы демонстрируют, что разработанное приложение обеспечивает полный функциональный цикл мониторинга здоровья — от регистрации до аналитики прогресса — в едином адаптивном веб-интерфейсе, свободном от платных ограничений. Практическая ценность подтверждается готовностью системы к развертыванию: приложение может быть запущено на любом VPS-сервере с помощью одной команды `docker compose up`, что обеспечивает воспроизводимое окружение и простоту сопровождения.
...
2. Флэнаган, Д. JavaScript: подробное руководство / Д. Флэнаган. – 7-е изд. – СПб.: БХВ-Петербург, 2021. – 704 с.
...
4. Mifflin, M. D. A new predictive equation for resting energy expenditure in healthy individuals / M. D. Mifflin [et al.] // The American Journal of Clinical Nutrition. – 1990. – Vol. 51, No. 2. – P. 241-247.

5. Аналитический центр НАФИ. Цифровой фитнес: исследование рынка фитнес-приложений в России 2024. – URL: https://nafi.ru/analytics/ (дата обращения: 15.03.2025). – Текст: электронный.

6. Всемирная организация здравоохранения. Физическая активность: информационный бюллетень. – URL: https://www.who.int/ru/news-room/fact-sheets/detail/physical-activity (дата обращения: 10.03.2025). – Текст: электронный.

7. Официальная документация Express.js. – URL: https://expressjs.com (дата обращения: 22.04.2025). – Текст: электронный.

8. Официальная документация MongoDB. – URL: https://docs.mongodb.com (дата обращения: 20.04.2025). – Текст: электронный.

9. Официальная документация React. – URL: https://react.dev (дата обращения: 15.04.2025). – Текст: электронный.

10. Axios HTTP Client Documentation. – URL: https://axios-http.com/docs/intro (дата обращения: 08.04.2025). – Текст: электронный.

11. bcrypt: Password Hashing Function — NIST SP 800-132. – URL: https://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-132.pdf (дата обращения: 10.05.2025). – Текст: электронный.

12. Chart.js Documentation. – URL: https://www.chartjs.org/docs/latest/ (дата обращения: 05.05.2025). – Текст: электронный.

13. Day.js Documentation. – URL: https://day.js.org/docs/en/installation/installation (дата обращения: 12.04.2025). – Текст: электронный.

14. JSON Web Token (JWT) — RFC 7519. – URL: https://jwt.io/introduction (дата обращения: 25.04.2025). – Текст: электронный.

15. Mongoose Documentation. – URL: https://mongoosejs.com/docs/ (дата обращения: 18.04.2025). – Текст: электронный.
...
17. React Router Documentation. – URL: https://reactrouter.com/en/main (дата обращения: 10.04.2025). – Текст: электронный.

18. Tailwind CSS Documentation. – URL: https://tailwindcss.com/docs (дата обращения: 27.04.2025). – Текст: электронный.
...
Листинг Б.3. Контекст локализации (client/src/context/LanguageContext.js) — фрагмент
...
// Словарь переводов (фрагмент)
const translations = {
    ru: {
        nav_home: 'Главная', nav_logout: 'Выйти',
        login_title: 'Добро пожаловать', login_submit: 'Войти',
        home_greeting: 'Привет,', home_kcal: 'ккал',
        toggle_lang: 'EN',
        // ... полный словарь в исходном коде
    },
    en: {
        nav_home: 'Home', nav_logout: 'Log out',
        login_title: 'Welcome back', login_submit: 'Sign in',
        home_greeting: 'Hello,', home_kcal: 'kcal',
        toggle_lang: 'RU',
        // ... полный словарь в исходном коде
    },
};
